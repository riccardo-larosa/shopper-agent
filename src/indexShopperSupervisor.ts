...
import { SHOPPER_SPRVR_TOOLS_LIST } from './tools'
...
const toolNode = new ToolNode(SHOPPER_SPRVR_TOOLS_LIST)

const callModel = async (state: ShopperState) => {
  ...
  const systemMessage = {
    ...
    content: `
      ...
      - For direct API calls, you can use execGetRequestTool and execPostRequestTool as before.
      - For cart operations, you should prefer using the enhancedCartTool, which can handle both GET and POST requests.
      
      The enhancedCartTool accepts natural language queries about cart operations like:
      - "add product ABC123 to cart"
      - "view my cart"  
      - "update quantity of product XYZ789 to 3"
      - "remove product DEF456 from cart"
      - "checkout my cart"
  ...
  const llmWithTools = llm.bindTools(SHOPPER_SPRVR_TOOLS_LIST)
...
const shouldContinue = (state: ShopperState) => {
